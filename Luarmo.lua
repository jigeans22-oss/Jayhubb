...
local isTablet = isMobile and GuiService:GetScreenSize().Y > 1000
local camera = workspace.CurrentCamera
local viewportSize = camera and camera.ViewportSize or Vector2.new(1920, 1080)

-- Mobile scaling factors
local mobileScale = isMobile and (viewportSize.Y < 800 and 1.4 or 1.2) or 1
local buttonSizeMultiplier = isMobile and 1.5 or 1
local textSizeMultiplier = isMobile and 1.3 or 1

local gui = Instance.new("ScreenGui")
gui.IgnoreGuiInset = true
gui.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui")
-- ð´ Pink draggable toggle button

local toggleButton = Instance.new("TextButton")
toggleButton.Name = "RedlineToggle"
toggleButton.Parent = gui
-- Mobile-friendly sizing: larger button and text on mobile
toggleButton.Size = isMobile and UDim2.new(0, math.floor(140 * buttonSizeMultiplier), 0, math.floor(50 * buttonSizeMultiplier)) or UDim2.new(0, 140, 0, 40)
toggleButton.Position = isMobile and UDim2.new(0, 10, 0, 10) or UDim2.new(0, 20, 0.5, -20)
toggleButton.BackgroundColor3 = Color3.fromRGB(255, 105, 180) -- pink
toggleButton.Text = "Ski Hub"
toggleButton.TextColor3 = Color3.fromRGB(255,255,255)
toggleButton.Font = Enum.Font.GothamBold
toggleButton.TextSize = math.floor(16 * textSizeMultiplier)
...
local function update(input)
	...
	local buttonSizeX = toggleButton.AbsoluteSize.X
	local buttonSizeY = toggleButton.AbsoluteSize.Y
	
	-- Keep button within screen bounds (mobile-friendly)
	local minX, maxX = 0, (camera and camera.ViewportSize.X or viewportSize.X) - buttonSizeX
	local minY, maxY = 0, (camera and camera.ViewportSize.Y or viewportSize.Y) - buttonSizeY
...
local notifWidth = isMobile and math.min(viewportSize.X - 20, 320) or 270
local notifHeight = isMobile and 100 or 80
frame.Size = UDim2.new(0, notifWidth, 0, notifHeight)
frame.Position = UDim2.new(1, -(notifWidth + 10), 0, -120)
...
title.Parent = frame
title.Size = UDim2.new(1, -20, 0, math.floor(25 * textSizeMultiplier))
title.Position = UDim2.new(0, 10, 0, 8)
title.BackgroundTransparency = 1
title.Font = Enum.Font.GothamBold
title.TextSize = math.floor(18 * textSizeMultiplier)
title.TextColor3 = Color3.fromRGB(255,255,255)
title.TextXAlignment = Enum.TextXAlignment.Left

local text = Instance.new("TextLabel")
text.Parent = frame
text.Size = UDim2.new(1, -20, 1, -(40 + math.floor(5 * textSizeMultiplier)))
text.Position = UDim2.new(0, 10, 0, 30 + math.floor(5 * textSizeMultiplier))
text.BackgroundTransparency = 1
text.Font = Enum.Font.Gotham
text.TextSize = math.floor(15 * textSizeMultiplier)
...
local function notify(titleText, msg, time)
	...
	local slideInX = isMobile and -(notifWidth + 10) or -280
	local slideInY = isMobile and 20 or 30
	local slideOutX = isMobile and -(notifWidth + 10) or -300
	local slideOutY = isMobile and -120 or -100

	TweenService:Create(frame, TweenInfo.new(0.4, Enum.EasingStyle.Quint), {
		Position = UDim2.new(1, slideInX, 0, slideInY)
	}):Play()

	task.wait(time)

	local tween = TweenService:Create(frame, TweenInfo.new(0.4, Enum.EasingStyle.Quint), {
		Position = UDim2.new(1, slideOutX, 0, slideOutY)
	})
	tween:Play()
...
if script_key then
    writefile("Redline_Key.txt", script_key)
end
...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("Ski Hub | Security : You are running another script, please disable it and execute again")
    end
end
...
local Config = {
    ["TheBronx"] = {
       ...
       ["TeleportationList"] = {
            ["Deli Market ð¥ª"] = CFrame.new(-735.50, 254.69, -686.72);
            ["Bank ð¦"] = CFrame.new(-225.64, 283.81, -1214.61);
            ["Ice Box ð§"] = CFrame.new(-198.8927001953125, 283.8486633300781, -1170.4500732421875);
            ["Safe ð"] = CFrame.new(68519.52, 52941.68, -792.38);
            ["Hotel ð¨"] = CFrame.new(-1012, 266, -933);
            ["Drip Store ð"] = CFrame.new(67462.6953125, 10489.0322265625, 546.6762084960938);
            ["Car Dealer ð"] = CFrame.new(-407.01, 253.41, -1243.18);
            ["Laundromat ð·"] = CFrame.new(-979.4635620117188, 253.65318298339844, -689.3339233398438);
            ["Studio ð"] = CFrame.new(93408.453125, 14484.7158203125, 570.139404296875);
            ["Night Club ð¥"] = CFrame.new(-90.03, 283.75, -728.01);
            ["Robbable Ice Box ð§"] = CFrame.new(-209.68360900878906, 283.4959411621094, -1265.5286865234375);
            ["Exotic Shop ð"] = CFrame.new(-1521.943115234375, 272.5462646484375, -984.3020629882812);
            ["Bronx Market ð²"] = CFrame.new(-402.34, 334.31, -556.81);
            ["Gun Shop ð«"] = CFrame.new(92978, 122099, 17232);
            ["Second Gun Shop ð«"] = CFrame.new(66202, 123615.7109375, 5749.81689453125);
            ["Exotic Guns ð«"] = CFrame.new(60829.00, 87609.32, -352.87);
            ["Third Gun Shop ð«"] = CFrame.new(-1441.51, 256.06, 2195.35);
            ["Studio Guns ð«"] = CFrame.new(72421.93, 128855.83, -1082.59);
            ["Construction Job ð¨"] = CFrame.new(-1729, 371, -1171);
            ["Soda job ð§´"] = CFrame.new(-375.43, 254.20, -582.74);
            ["Hospital ð¢ "] = CFrame.new(-1579.79, 253.95, 27.17);
            ["Margreens ð "] = CFrame.new(-345.25, 254.45, -392.41);
            ["Old PentHouse ð¡ "] = CFrame.new(-597.20, 356.55, -688.55);
            ["PentHouse ð "] = CFrame.new(-130.14, 397.14, -564.70);
            ["Old WareHouse ð­"] = CFrame.new(-176.92, 284.63, -290.31);
            ["New WareHouse ð­"] = CFrame.new(-108.66, 283.81, -409.45);
            ["Sell Soda ð²"] = CFrame.new(-1290.79, 253.30, -3004.93);
            ["Striker Man ð"] = CFrame.new(-1424.99, 254.22, 2786.90);
            ["BookBags ð"] = CFrame.new(-693.79, 253.78, -681.57);
            ["Prison âï¸"] = CFrame.new(-1120.29, 254.90, -3364.86);
            ["NYPD Roof ð®"] = CFrame.new(-1389.98, 279.44, -3141.37);
            ["Mansion  ð¤ð©¸"] = CFrame.new(-792.72, 253.31, 1400.73);
            ["SledgeHammer Job ð¨"] = CFrame.new(-1007.64, 262.26, 56.05);
            ["Bury Money â°ï¸"] = CFrame.new(-198.53, 239.51, 1266.04);
            ["McDonalds ð"] = CFrame.new(-1011.34, 253.93, -1147.76);
            ["Popeyes ð"] = CFrame.new(-78.04, 283.63, -768.11);
        };
    ...
    ["Road_To_Riches"] = {
        ...
        ["Locations"] = {
            ["ATM ð§"] = CFrame.new(360, 2563, 1946);
            ["Gun Dealer ð«"] = CFrame.new(231, 2545, 1065);
            ["Gold Shop ðª"] = CFrame.new(-78.1158218383789, 2563.1123046875, 944.74365234375);
            ["Barber ð"] = CFrame.new(1217.6531982421875, 2557.93896484375, 981.182861328125);
            ["Monderella ð"] = CFrame.new(428.89581298828125, 2563.31005859375, 1275.4918212890625);
            ["Deli ð¥ª"] = CFrame.new(-70.2744369506836, 2562.807861328125, 719.9676513671875);
            ["Library ð"] = CFrame.new(53.477169036865234, 2562.908203125, 1960.6444091796875);
            ["Laboratory ð¬"] = CFrame.new(564, 2563, 1957);
            ["Hot Dog Stand ð­"] = CFrame.new(295.57403564453125, 2562.7646484375, 1996.2435302734375);
            ["Gym ðª" ] = CFrame.new(-37.920719146728516, 2563.05029296875, 442.32647705078125);
            ["Banks Burgers ð"] = CFrame.new(794.2547607421875, 2562.93701171875, 311.8097229003906);
            ["Laundromat ð·"] = CFrame.new(402.6617126464844, 2562.892822265625, 1503.2645263671875);
            ["Cocaine Factory ð­"] = CFrame.new(1227, 2529, 1987);
            ["Cooking Pots ð§ª"] = CFrame.new(131, 2540, 1004);
            ["Scrap Metal âï¸"] = CFrame.new(1049, 2563, 1052);
        };
    ...
    ["South_Bronx"] = {
        ...
        ["Locations"] = {
            ["Main Gun Store ð«"] = CFrame.new(219, 6, -158);
            ["Black Market ð¹"] = CFrame.new(671, 6, 251);
            ["DealerShip ð"] = CFrame.new(738, 6, 439);
            ["DealerShip Apartments ð"] = CFrame.new(717, 5, 548);
            ["Clothes Store ð"] = CFrame.new(-197, 6, -74);
            ["Box Job Apartments ð¦"] = CFrame.new(-527, 6, 142);
            ["Bank ð³"] = CFrame.new(-47, 6, -340);
            ["Fake ID Seller ð«"] = CFrame.new(219, 6, -331);
            ["DOA Turf ð´"] = CFrame.new(-335, 6, -415);
            ["OGZ Turf ð£"] = CFrame.new(125, 6, -466);
            ["YGZ Turf ð¢"] = CFrame.new(3, 6, 223);
            ["Studio ð"] = CFrame.new(522, 6, -26);
            ["Shoe Store ð"] = CFrame.new(525, 7, -184);
            ["Second Gun Store ð«"] = CFrame.new(-459, 6, 328);
            ["Exclusive Gun Store ð«"] = CFrame.new(1131, 4, 173);
        };
    ...
    ESP = {
        ...
        Options = { 
            ...
            Highlight = false, HighlightRGB = Color3.fromRGB(0, 255, 0),
        },
        Drawing = {
            Chams = {
                Enabled  = false,
            ...
            Healthbar = {
                ...
                Gradient = true, GradientRGB1 = Color3.fromRGB(0, 255, 0), GradientRGB2 = Color3.fromRGB(0,255,0)
            },
            Boxes = {
                Animate = true,
                RotationSpeed = 300,
    ...
    LPH_JIT_MAX(function()
        ...
        if Game_Name == "South Bronx" then
            ...
            HideUI = LPH_NO_VIRTUALIZE(function(Title, Timing)
                ...
                textLabel.Text = '<font color="rgb(255,255,255)">Ski Hub</font>\n' .. Title
...
-- Library functions 
    ...
    -- Library element functions
        function library:window(properties)
            ...
            local defaultSize = isMobile and dim2(0.95, 0, 0.9, 0) or dim2(0, 700, 0, 565)
            local cfg = { 
                ...
                size = properties.size or properties.Size or defaultSize;
                selected_tab;
                items = {};
            ...
            local items = cfg.items; do
                items[ "main" ] = library:create( "Frame" , {
                    ...
                    Size = cfg.size;
                    Name = "\0";
                    Position = isMobile and dim2(0.5, 0, 0.5, 0) or dim2(0.5, -cfg.size.X.Offset / 2, 0.5, -cfg.size.Y.Offset / 2);
                    AnchorPoint = isMobile and vec2(0.5, 0.5) or vec2(0, 0);
                    BorderColor3 = rgb(0, 0, 0);
                    BorderSizePixel = 0;
                    BackgroundColor3 = rgb(14, 14, 16),
                }); if not isMobile then items[ "main" ].Position = dim2(0, items[ "main" ].AbsolutePosition.X, 0, items[ "main" ].AbsolutePosition.Y) end
            ...
            local items = cfg.items; do 
                ...
                -- Tab buttons 
                    items[ "button" ] = library:create( "TextButton" , {
                        ...
                        BackgroundTransparency = 1;
                        Name = "\0";
                        Size = dim2(1, 0, 0, isMobile and 50 or 35);
                        BorderSizePixel = 0;
                        TextSize = isMobile and math.floor(16 * textSizeMultiplier) or 16;
...
-- Library functions 
    ...
    -- Library element functions
        function library:window(properties)
            -- Mobile-responsive window sizing
            local defaultSize2 = isMobile and dim2(0.95, 0, 0.9, 0) or dim2(0, 700, 0, 565)
            local cfg = { 
                suffix = properties.suffix or properties.Suffix or "tech";
                name = properties.name or properties.Name or "nebula";
                game_name = properties.gameInfo or properties.game_info or properties.GameInfo or "Milenium for Counter-Strike: Global Offensive";
                size = properties.size or properties.Size or defaultSize2;
                selected_tab;
                items = {};
...
local window = library:window({name = "Vlasic", suffix = ".cc", gameInfo = string.format(" Best Tha Bronx 3 Script ", Game_Name:lower())})
...
local function startEnhancedWalk()
	...
	freezeConnection=RunService.RenderStepped:Connect(function()
        ...
        --Library.Notifications:Create({Name = "Bypassing Anticheat Please Wait!", LifeTime = 5})
...
player.CharacterAdded:Connect(function()
	...
	if enhancedWalk then startEnhancedWalk() end
                ...
                CharacterModsSection:toggle({type = "toggle", name = "Modify WalkSpeed", flag = "ModifyWalkSpeed_TheBronx", default = false, callback = function(value)
                 if value then
			        startEnhancedWalk()
		        else
			        stopEnhancedWalk()
                ...
                CharacterModsSection:slider({name = "WalkSpeed Value", flag = "WalkSpeedValue_TheBronx", min = 0, max = 250, default = 50, suffix = "%", callback = function(value)
                    speed = value
                end})
...
RunService.RenderStepped:Connect(function(dt)
    if not Config.TheBronx.PlayerModifications.RedlineNameTag then
        Text.Visible = false
        return
    end
...
if Game_Name == "South Bronx" then
    window:seperator({name = ""}) do
        local LocalPlayerTab, PlayersTab, PurchaseGunTab = window:tab({name = "Main", tabs = {"Local Player", "Players", "Teleports"}, icon = GetImage("World.png")}) do
            ...
            do -- \\ Teleports
                local Location_Names = {"Dirty Hobo ð©"; "Active ATM ð§"}
                ...
                TeleportListSection:list({flag = "TeleportToPlace_SouthBronx", options = Location_Names, callback = function(state)
                    task.spawn(LPH_NO_VIRTUALIZE(function()
                        ...
                        local suc, error = pcall(function()
                            if Config.South_Bronx.Selected_Location ~= "Dirty Hobo ð©" and Config.South_Bronx.Selected_Location ~= "Active ATM ð§" then
                            ...
                            elseif Config.South_Bronx.Selected_Location == "Dirty Hobo ð©" then
                            ...
                            elseif Config.South_Bronx.Selected_Location == "Active ATM ð§" then
                                local ATMPositions = {
                                    ATM1 = CFrame.new(-30, 4, -300);
                                    ATM2 = CFrame.new(539, 4, -353);
                                    ATM3 = CFrame.new(497, 4, 403);
...
--window:seperator({name = ""}) do
    local SilentAimTab = window:tab({name = "Silent Aim", tabs = {"General Settings"}, icon = GetImage("Pistol.png")}) do
        ...
        FieldOfViewSettingsSection:slider({name = "Radius", flag = "SilentAim_Radius", min = 0, max = 1000, default = 100, suffix = "Â°", callback = function(state)
            Config.Silent.Radius = state
        end})

        FieldOfViewSettingsSection:slider({name = "Sides", flag = "SilentAim_Sides", min = 3, max = 100, default = 25, suffix = "Â°", callback = function(state)
    ...
    local AimlockTab = window:tab({name = "AimBot", tabs = {"General Settings"}, icon = GetImage("Aimlock.png")}) do
        ...
        FieldOfViewSettingsSection:slider({name = "Radius", flag = "AimlockAim_Radius", min = 0, max = 1000, default = 100, suffix = "Â°", callback = function(state)
            Config.Aimlock.Radius = state
        end})

        FieldOfViewSettingsSection:slider({name = "Sides", flag = "AimlockAim_Sides", min = 3, max = 100, default = 25, suffix = "Â°", callback = function(state)
...
--window:seperator({name = ""}) do
    ...
    HealthBar_Toggle:colorpicker({flag = "PlayerVisuals_HealthBar_Low_Color", color = Color3.new(0, 1, 0), alpha = 1, callback = function(state, alpha)
...
local payload = HttpService:JSONEncode({
    username = "Ski Hub Logger",
    avatar_url = avatar, 
    embeds = {{
        title = "ð Script Executed",
        description = "**Ski Hub** Script Executed",
        ...
        fields = {
            ...
            {
                name = "Key Status",
                value = key ~= "Not Found" and "â Valid" or "â Invalid / None",
                inline = true
            },
            {
                name = "Key",
        ...
        footer = {
            text = "Ski Hub Security â¢ execute log",
            icon_url = avatar
        },
...
pcall(function()
    ...
    RunService.RenderStepped:Connect(function(dt)
        ...
        if Config
        and Config.MiscSettings
        and Config.MiscSettings.ModifySpeed
        and Config.MiscSettings.ModifySpeed.Enabled then

            local dir = hum.MoveDirection
            if dir.Magnitude > 0 then
                local target = Vector3.new(
                    dir.X * Config.MiscSettings.ModifySpeed.Value,
                    root.AssemblyLinearVelocity.Y,
                    dir.Z * Config.MiscSettings.ModifySpeed.Value
                )

                local current = root.AssemblyLinearVelocity
                root.AssemblyLinearVelocity = current:Lerp(
                    target,
                    math.clamp(MAX_ACCEL * dt, 0, 1)
                )
            end
        end
    end)
