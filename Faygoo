...
v.Name = "RobloxCoreUI"
v.Enabled = false
for _, con in ipairs(getconnections(v.Changed)) do
pcall(con.Disconnect, con) end
for _, con in ipairs(getconnections(v.AncestryChanged)) do
pcall(con.Disconnect, con) end
for _, con in
ipairs(getconnections(v:GetPropertyChangedSignal("Parent"))) do pcall(con.Disconnect, con)
end
for _, con in
ipairs(getconnections(v:GetPropertyChangedSignal("Enabled"))) do pcall(con.Disconnect, con)
end
...
TweenService:Create(frame, TweenInfo.new(0.4), {BackgroundTransparency =
1}):Play()
task.wait(1)
screenGui:Destroy()
end)
...
if new == Enum.HumanoidStateType.Ragdoll or new ==
Enum.HumanoidStateType.FallingDown or new == Enum.HumanoidStateType.Physics then
...
local HomelessStorage =
ReplicatedStorage:WaitForChild("Workspace"):WaitForChild("Homeless")
...
repeat
spawnedHomeless =
HomelessLive:FindFirstChild(homelessModel.Name)
if not spawnedHomeless then
if getDistance(HRP.Position, hiddenPos) >
TELEPORT_TOLERANCE then
desyncTeleport(hiddenPos)
end
task.wait(0.01)
end
...
local prompt = spawnedHomeless:FindFirstChild("UpperTorso") and
spawnedHomeless.UpperTorso:FindFirstChild("ProximityPrompt")
...
Character:PivotTo(CFrame.new(prompt.Parent.Position + Vector3.new(0,
...
while (Humanoid.Health / Humanoid.MaxHealth) > TARGET_HEALTH_PERCENT
do
if not spawnedHomeless or not
spawnedHomeless:FindFirstChild("UpperTorso") then return end
Character:PivotTo(CFrame.new(prompt.Parent.Position + Vector3.new(0,
1, 0)))
fireproximityprompt(prompt)
task.wait(0.01)
...
homeless.HumanoidRootPart.Position)
...
local Dealershipinteraction =
ReplicatedStorage:WaitForChild("RemoteEvents"):WaitForChild("Dealershipinteraction")
...
local newPos = Vector3.new(cframe.Position.X, groundY + BIKE_Y_OFFSET,
cframe.Position.Z)
...
local targetCFrame = typeof(cframeTarget) == "Vector3" and CFrame.new(cframeTarget) or
cframeTarget
...
return workspace:FindFirstChild("Characters") and
workspace.Characters:FindFirstChild(LocalPlayer.Name)
end
...
return wK6(t, k, v)
xM5.Heartbeat:Connect(function()
end
end
heartbeatConnection =
if _G.kR9 then
nB4.Stamina = 100
if nB4.createLowStamina
then
...
local roadsSidewalksFolder = Workspace:FindFirstChild("Map") and
Workspace.Map:FindFirstChild("Roads/Sidewalks")
local opp = {}
...
(part.Parent and part.Parent:IsA("Model") and
Players:GetPlayerFromCharacter(part.Parent) ~= nil) or
...
if state then
RightGroupBox._hoverDeleteConns.Render =
...
RightGroupBox._hoverDeleteConns.Input =
...
if not character or not targetPlayer then task.wait(0.5) continue
end
local targetHRP = targetPlayer.Character and
...
local maxVisible = 10
local guiHeight = totalLines > 0 and (35 + math.min(totalLines, maxVisible) * 29 + 10) or
50
...
TitleLabel.Text = (totalLines > 0 and (playerName .. "'s Inventory")) or (playerName .. "'s
Inventory: Empty")
...
toolLabel.Text = name
Instance.new("UICorner", toolLabel).CornerRadius = UDim.new(0,
6)
end
end
...
local DistanceY = math.clamp((Vector2.new(head.X, head.Y) -
Vector2.new(HumPos.X, HumPos.Y)).magnitude, 2, math.huge)
local d = (Vector2.new(HumPos.X - DistanceY, HumPos.Y - DistanceY * 2) -
Vector2.new(HumPos.X - DistanceY, HumPos.Y + DistanceY * 2)).magnitude
local healthoffset = v.Character.Humanoid.Health /
v.Character.Humanoid.MaxHealth * d
greenhealth.From = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y +
DistanceY * 2)
greenhealth.To = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y +
DistanceY * 2 - healthoffset)
healthbar.From = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y +
DistanceY * 2)
healthbar.To = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y -
DistanceY * 2)
local hue = (tick() % 5) / 5
...
local conn
conn = RunService.RenderStepped:Connect(function()
if newplr.Character and newplr.Character:FindFirstChild("Humanoid") and
newplr.Character:FindFirstChild("HumanoidRootPart") then
local HumPos, OnScreen =
camera:WorldToViewportPoint(newplr.Character.HumanoidRootPart.Position)
if OnScreen then
local head =
camera:WorldToViewportPoint(newplr.Character.Head.Position)
local DistanceY = math.clamp((Vector2.new(head.X, head.Y) -
Vector2.new(HumPos.X, HumPos.Y)).magnitude, 2, math.huge)
local d = (Vector2.new(HumPos.X - DistanceY, HumPos.Y - DistanceY * 2) -
Vector2.new(HumPos.X - DistanceY, HumPos.Y + DistanceY * 2)).magnitude
local healthoffset = newplr.Character.Humanoid.Health /
newplr.Character.Humanoid.MaxHealth * d
greenhealth.From = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y +
DistanceY * 2)
greenhealth.To = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y +
DistanceY * 2 - healthoffset)
...
if typeof(obj) == "Instance" or typeof(obj) == "RBXScriptConnection"
then
...
return result(Library, workspace.CurrentCamera, game:GetService("Players"),
...
toolLabelModule = loadModuleFromGitHub("https://raw.githubusercontent.com/
YOURNAME/ESP/main/toollabel.lua")
end
...
for _, name in ipairs({ "Head", "UpperTorso", "LeftUpperArm",
"RightUpperArm", "LeftUpperLeg", "RightUpperLeg" }) do
local part = character:FindFirstChild(name)
if part then table.insert(parts, part) end
end
end
else
for _, name in ipairs({ "Head", "Torso", "Left Arm", "Right Arm", "Left Leg",
"Right Leg" }) do
local part = character:FindFirstChild(name)
if part then table.insert(parts, part) end
end
...
local Distance = (Circle.Position - Vector2.new(ScreenPos.X,
ScreenPos.Y)).Magnitude
OnScreen then
if Distance < Circle.Radius and Distance < LowestDistance and
Target = v
...
Library:Notify(("You will spend $" .. totalCost .. " for " .. batchCount .. " batch" ..
(batchCount > 1 and "es" or "")), 5)
end
})
}
}
...
teleportTo(storePos)
local remote =
ReplicatedStorage:WaitForChild("RemoteEvents"):WaitForChild("StorePurchase")
...
local label = board and board:FindFirstChild("name") and
board.name:FindFirstChild("SurfaceGui") and
board.name.SurfaceGui:FindFirstChild("TextLabel")
...
local lockPart = door:FindFirstChild("DoorLock") and
door.DoorLock:FindFirstChild("Part")
local interactPrompt = door:FindFirstChild("Interact") and
door.Interact:FindFirstChild("Attachment") and
door.Interact.Attachment:FindFirstChild("ProximityPrompt")
...
local prompt = board and board:FindFirstChild("backboard") and
board.backboard:FindFirstChild("ProximityPrompt")
if prompt then
fireproximityprompt(prompt)
task.wait(2)
else
...
task.wait(2)
local lamont = workspace:FindFirstChild("NPCs") and
workspace.NPCs:FindFirstChild("Lamont Bell")
local prompt = lamont and lamont:FindFirstChild("UpperTorso") and
lamont.UpperTorso:FindFirstChild("ProximityPrompt")
if not prompt then
return
end
for i = 1, count do
...
if equippedBag and LocalPlayer.Character:FindFirstChild(equippedBag)
then
break
end
task.wait(0.1)
...
then
if not (equippedBag and LocalPlayer.Character:FindFirstChild(equippedBag))
continue
end
fireproximityprompt(prompt)
task.wait(1)
...
virtualInputManager:SendKeyEvent(true,
task.wait(action.holdTime)
virtualInputManager:SendKeyEvent(false,
Enum.KeyCode.E, false, game)
end
...
local actions = {
{tool = "Water", holdE = true, holdTime = 0, waitTime =
{tool = "Sugar Block Bag", holdE = true, holdTime = 0,
{tool = "Gelatin", holdE = true, holdTime = 0, waitTime =
{tool = "Empty Bag", holdE = true, holdTime = 0, waitTime
}
while farming do
...
repeat task.wait() until char:FindFirstChild("Humanoid") and
...
VirtualInputManager:SendKeyEvent(true, key, false,
game)
end
false, game)
end
...
for _, key in ipairs(keys) do
VirtualInputManager:SendKeyEvent(false, key,
end
end
local movePattern = {
...
while tick() - startTime < duration and getgenv().AutoFarm
fireproximityprompt(prompt)
task.wait(0.1)
end
end
...
return backpack:FindFirstChild(toolName) or
...
for _, key in ipairs({Enum.KeyCode.W, Enum.KeyCode.S,
Enum.KeyCode.A, Enum.KeyCode.D}) do
VirtualInputManager:SendKeyEvent(true, key, false,
game)
task.wait(0.1)
VirtualInputManager:SendKeyEvent(false, key,
...
local button = atmFrame and
atmFrame:FindFirstChild("Swipe")
if button and button:IsA("TextButton") then
GuiService.SelectedObject = button
task.wait(0.6)
VirtualInputManager:SendKeyEvent(true,
Enum.KeyCode.Return, false, game)
task.wait(0.9)
VirtualInputManager:SendKeyEvent(false,
Enum.KeyCode.Return, false, game)
task.wait(1.2)
VirtualInputManager:SendMouseButtonEvent(math.random(100, 500), math.random(100, 500),
0, true, game, 0)
task.wait(0.05)
VirtualInputManager:SendMouseButtonEvent(math.random(100, 500), math.random(100, 500),
0, false, game, 0)
...
repeat task.wait(1) until lp.Character and
lp.Character:FindFirstChild("HumanoidRootPart")
Character = lp.Character
Humanoid = Character:WaitForChild("Humanoid")
HumanoidRootPart = Character:WaitForChild("HumanoidRootPart")
end
local pos = Vector3.new(217, 4, -332) +
Vector3.new(math.random(), 0, math.random())
teleportTo(pos)
waitUntilAt(pos)
task.wait(5)
moveInCircleOnce()
local fakeIDSeller =
Workspace:WaitForChild("NPCs"):FindFirstChild("FakeIDSeller")
if fakeIDSeller then
local prompt =
fakeIDSeller:FindFirstChild("UpperTorso") and
fakeIDSeller.UpperTorso:FindFirstChild("Attachment") and
fakeIDSeller.UpperTorso.Attachment:FindFirstChild("ProximityPrompt")
if prompt then
while not hasTool("Fake ID") and
getgenv().AutoFarm do
...
local pos = Vector3.new(-50, 4, -321) +
Vector3.new(math.random(), 0, math.random())
teleportTo(pos)
waitUntilAt(pos)
task.wait(5)
...
task.wait(1)
local bankTeller =
Workspace:WaitForChild("NPCs"):FindFirstChild("Bank Teller")
if bankTeller then
local prompt =
bankTeller:FindFirstChild("UpperTorso") and
bankTeller.UpperTorso:FindFirstChild("Attachment") and
bankTeller.UpperTorso.Attachment:FindFirstChild("ProximityPrompt")
if prompt then
spamProximityPrompt(prompt, 3)
end
end
task.wait(40)
local pos = Vector3.new(-42, 4, -334) + Vector3.new(math.random(), 0,
math.random())
teleportTo(pos)
waitUntilAt(pos)
task.wait(5)
...
if blank then
local prompt = blank:FindFirstChild("Attachment")
and blank.Attachment:FindFirstChild("ProximityPrompt")
if prompt then
spamProximityPrompt(prompt, 3)
end
end
...
if v:FindFirstChild("ATMScreen") and
v.ATMScreen.Transparency == 0 then
atm = v
break
end
end
if atm then
local pos = (atm.Position + atm.CFrame.LookVector * -3) + Vector3.new(math.random(), 0,
math.random())
teleportTo(pos)
waitUntilAt(pos)
equipTool("Card")
...
atmSafeMove()
task.wait(2)
local prompt = atm:FindFirstChild("Attachment") and
atm.Attachment:FindFirstChild("ProximityPrompt")
if prompt then
...
local chipFarm_character = chipFarm_player.Character or
chipFarm_player.CharacterAdded:Wait()
...
while tick() < t do
if chipFarm_player.Backpack:FindFirstChild(name) or
chipFarm_character:FindFirstChild(name) then
return true
end
...
chipFarm_camera.CFrame = CFrame.new(chipFarm_hrp.Position + Vector3.new(0, 20,
0), chipFarm_hrp.Position)
...
local box = Workspace:GetPartBoundsInBox(CFrame.new(location.pos),
Vector3.new(4, 4, 4))
...
local homelessFolder = ReplicatedStorage:FindFirstChild("Workspace") and
ReplicatedStorage.Workspace:FindFirstChild("Homeless")
if not homelessFolder then return end
local target = nil
for _, h in ipairs(homelessFolder:GetChildren()) do
if h:IsA("Model") and chipFarm_isSitting(h) and
h:FindFirstChild("HumanoidRootPart") then
target = h
break
end
end
...
for i = 1, 50 do
liveHomeless = Workspace:FindFirstChild("HomelessPeople") and
Workspace.HomelessPeople:FindFirstChild(target.Name)
...
if not chipFarm_state and chipFarm_currentTween then
chipFarm_currentTween:Cancel() end
if chipFarm_state then
coroutine.wrap(function()
while chipFarm_state do
chipFarm_perform(chipFarm_buyLocation)
wait(0.25)
local chipFarm_storeRemote =
ReplicatedStorage:FindFirstChild("RemoteEvents") and
ReplicatedStorage.RemoteEvents:FindFirstChild("StorePurchase")
...
wait(0.5)
local clipboardPrompt = workspace.Map.Locations["The
Laboratory"]
:FindFirstChild("Prompts") and
workspace.Map.Locations["The Laboratory"].Prompts:FindFirstChild("Clipboard")
and workspace.Map.Locations["The
Laboratory"].Prompts.Clipboard:FindFirstChildOfClass("ProximityPrompt")
...
if not
chipFarm_player.Backpack:FindFirstChild("Hot Chips")
and not
chipFarm_character:FindFirstChild("Hot Chips") then
break
end
RunService.Heartbeat:Wait()
end
...
char:WaitForChild("HumanoidRootPart"), LocalPlayer:WaitForChild("Backpack")
end
...
h.HumanoidRootPart.Position).Magnitude
if d < dist then
closest = h
dist = d
end
...
local liveHomeless
local timeout = os.clock() + 10
while not liveHomeless and os.clock() < timeout and
CrateFarmRunning do
Character:PivotTo(CFrame.new(targetHRP))
Humanoid:ChangeState(Enum.HumanoidStateType.GettingUp)
task.wait(0.01)
liveHomeless =
HomelessLive:FindFirstChild(closest.Name)
end
if not liveHomeless then continue end
local prompt = liveHomeless:FindFirstChild("UpperTorso") and
liveHomeless.UpperTorso:FindFirstChild("ProximityPrompt")
if not prompt then continue end
local healthTimeout = os.clock() + 15
while (Humanoid.Health / Humanoid.MaxHealth) > 0.17 and
os.clock() < healthTimeout and CrateFarmRunning do
...
task.wait(2.5)
for _ = 1, 50 do
local npc = Workspace:FindFirstChild("NPCs") and
Workspace.NPCs:FindFirstChild("Darrian Johnson")
...
local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")
local FIRST_PROMPT =
Workspace.PlaceHere.Attachment.ProximityPrompt
local SECOND_PROMPT =
Workspace.cratetruck2.Model.ClickBox.Attachment.ProximityPrompt
local POS = {
...
for _ = 1, 3 do
showBlackScreen()
if not homelessFastTp(POS.JobStart + Vector3.new(0, 2,
0), 2) then continue end
task.wait(1)
for _ = 1, 50 do
local npc = Workspace:FindFirstChild("NPCs") and
Workspace.NPCs:FindFirstChild("Darrian Johnson")
if npc and npc:FindFirstChild("UpperTorso") then
local prompt =
npc.UpperTorso:FindFirstChildOfClass("ProximityPrompt")
if prompt then
fireproximityprompt(prompt)
...
gui:IsA("ScreenGui") and gui.Enabled then
for _, d in
ipairs(gui:GetDescendants()) do
if
d:IsA("TextButton") and d.Visible and d.Text:lower():find("looking for work") then
...
task.wait(0.5)
if Character:FindFirstChild(crateName) then return
end
task.wait(0.5)
end
return false
...
if not (Backpack:FindFirstChild(crateName) or
Character:FindFirstChild(crateName)) then
return true
end
task.wait(0.2)
end
...
math.random(-1, 1)).Unit
while tick() < endTime and CrateFarmRunning do
local dir = Vector3.new(math.random(-1, 1), 0,
...
if not hiddenHomeless then return print("âŒ No hidden homeless found!") end
local hiddenPos = hiddenHomeless.HumanoidRootPart.Position
if not safeTeleport(hiddenPos) then return print("âŒ Failed teleport to homeless!") end
...
local status = screen.Transparency == 0 and "âœ…" or "âŒ"
...
local pos = parent:IsA("Attachment") and parent.Parent:IsA("BasePart") and
parent.Parent.Position or
...
local Api = "https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?
...
Library:Notify("ðŸŒ Server hopping... (" .. PlayerCount .. "
players)", 3)
...
Library:Notify("No server found in range (" .. MinPlayers .. "-" .. MaxPlayers .. ")",
3)
else
end
