...
local window
if Game_Name ~= "The Bronx" then
    window = library:window({name = "bronx", suffix = ".lol", gameInfo = string.format("bronx.lol : %s", Game_Name:lower())})
end

if false and Game_Name == "The Bronx" then
...
-- // Nameless Hub | Synapse-style UI (Delta compatible, draggable, open/close) for The Bronx

do
    if Game_Name ~= "The Bronx" then return end
    ...
    local Config = Config or getgenv().Config
    if not Config then
        Config = {}
        getgenv().Config = Config
    end

    Config.TheBronx = Config.TheBronx or {}
    Config.WorldVisuals = Config.WorldVisuals or {}
    Config.MiscSettings = Config.MiscSettings or {}
    Config.Aimlock = Config.Aimlock or {}
    Config.Silent = Config.Silent or {}
    Config.ESP = Config.ESP or {}

    local TheBronx = Config.TheBronx
    local WorldVisuals = Config.WorldVisuals
    local MiscSettings = Config.MiscSettings
    ...
    local function safecall(fn, ...)
        if typeof(fn) == "function" then
            ...
            if not ok then
                warn("[Nameless Hub] UI callback error:", err)
            end
        end
    end
    ...
    MasterGui.Name = "NamelessHub_SynapseStyle"
    MasterGui.ResetOnSpawn = false
    MasterGui.IgnoreGuiInset = true
    MasterGui.Parent = gui_parent
    ...
    OpenButton.BorderColor3 = Color3.fromRGB(60, 60, 60)
    OpenButton.AutoButtonColor = true
    OpenButton.Font = Enum.Font.GothamBold
    OpenButton.Text = "Open Nameless Hub"
    OpenButton.TextSize = 13
    OpenButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    OpenButton.Parent = MasterGui
    ...
    TopBarLine.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    ...
    Title.Text = "Nameless Hub  |  The Bronx"
    Title.TextSize = 14
    Title.TextColor3 = Color3.fromRGB(220, 220, 220)
    Title.Parent = TopBar

    -- Open / Close button in top-right
    ...
    TabsFrame.Name = "Tabs"
    TabsFrame.Size = UDim2.new(0, 140, 1, -32)
    ...
    ContentFrame.Position = UDim2.new(0, 140, 0, 32)
    ContentFrame.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
    ContentFrame.BorderColor3 = Color3.fromRGB(50, 50, 50)
    ...
    local function createPage(name)
        ...
        TabButton.Size = UDim2.new(1, -2, 0, 26)
    ...
    local function addToggle(parent, text, default, callback)
        ...
        Button.Text = (default and "[ON]  " or "[OFF] ") .. text
        Button.TextSize = 13
    ...
    local function addNumberBox(parent, text, default, callback)
        local Holder = Instance.new("Frame")
        Holder.Name = text .. "_NumberHolder"
    ...
    local GamePage, GameTabButton         = createPage("Game")
    local CombatPage, CombatTabButton     = createPage("Combat")
    local VisualsPage, VisualsTabButton   = createPage("Visuals")
    local MovementPage, MovementTabButton = createPage("Movement")
    local FarmsPage, FarmsTabButton       = createPage("Farms")
    local TeleportPage, TeleportTabButton = createPage("Teleports")
    local MiscPage, MiscTabButton         = createPage("Misc")

    -- Select default tab
    CombatTabButton.MouseButton1Click:Fire()

    -- GAME TAB (placeholder / general info)
    addSectionTitle(GamePage, "Game Info")
    local GameLabel = Instance.new("TextLabel")
    GameLabel.BackgroundTransparency = 1
    GameLabel.Size = UDim2.new(1, -4, 0, 18)
    GameLabel.Font = Enum.Font.Gotham
    GameLabel.TextXAlignment = Enum.TextXAlignment.Left
    GameLabel.Text = "bronx.lol remastered | Nameless Hub"
    GameLabel.TextSize = 12
    GameLabel.TextColor3 = Color3.fromRGB(210, 210, 210)
    GameLabel.Parent = GamePage

    -- COMBAT (Modifications + Aimlock/Silent)
    addSectionTitle(CombatPage, "Weapon Modifications")

    if type(TheBronx._Modifications) == "table" then
        for key, val in pairs(TheBronx._Modifications) do
            local label = prettify(key)
            if type(val) == "boolean" then
                addToggle(CombatPage, label, val, function(state)
                    TheBronx._Modifications[key] = state
                end)
            elseif type(val) == "number" then
                addNumberBox(CombatPage, label, val, function(v)
                    TheBronx._Modifications[key] = v
                end)
            end
        end
    end

    addSectionTitle(CombatPage, "Aimlock")
    addToggle(CombatPage, "Aimlock Enabled", Aimlock.Enabled or false, function(state)
        Aimlock.Enabled = state
    end)
    addNumberBox(CombatPage, "Aimlock FOV", Aimlock.Radius or 100, function(v)
        Aimlock.Radius = v
    end)

    addSectionTitle(CombatPage, "Silent Aim")
    addToggle(CombatPage, "Silent Aim Enabled", Silent.Enabled or false, function(state)
        Silent.Enabled = state
    end)
    addNumberBox(CombatPage, "Silent FOV", Silent.Radius or 100, function(v)
        Silent.Radius = v
    end)

    -- VISUALS TAB
    addSectionTitle(VisualsPage, "ESP")
    addToggle(VisualsPage, "ESP Enabled", ESP.Enabled or false, function(state)
        ESP.Enabled = state
        Config.ESP = ESP
        safecall(RefreshAllElements)
    end)

    addSectionTitle(VisualsPage, "World Visuals")
    addToggle(VisualsPage, "Fullbright", WorldVisuals.Fullbright or false, function(state)
        WorldVisuals.Fullbright = state
    end)
    addToggle(VisualsPage, "Custom FOV", WorldVisuals.FieldOfViewEnabled or false, function(state)
        WorldVisuals.FieldOfViewEnabled = state
    end)
    addNumberBox(VisualsPage, "FOV Value", WorldVisuals.FieldOfViewValue or 70, function(v)
        WorldVisuals.FieldOfViewValue = v
    end)

    -- MOVEMENT TAB
    addSectionTitle(MovementPage, "Movement Mods")
    MiscSettings.ModifySpeed = MiscSettings.ModifySpeed or {Enabled = false, Value = 50}
    MiscSettings.ModifyJump  = MiscSettings.ModifyJump  or {Enabled = false, Value = 50}

    addToggle(MovementPage, "Modify WalkSpeed", MiscSettings.ModifySpeed.Enabled, function(state)
        MiscSettings.ModifySpeed.Enabled = state
    end)
    addNumberBox(MovementPage, "WalkSpeed %", MiscSettings.ModifySpeed.Value or 50, function(v)
        MiscSettings.ModifySpeed.Value = v
    end)

    addToggle(MovementPage, "Modify Jump", MiscSettings.ModifyJump.Enabled, function(state)
        MiscSettings.ModifyJump.Enabled = state
    end)
    addNumberBox(MovementPage, "Jump Power", MiscSettings.ModifyJump.Value or 50, function(v)
        MiscSettings.ModifyJump.Value = v
    end)

    -- FARMS TAB (everything in TheBronx.Farms)
    if type(TheBronx.Farms) == "table" then
        addSectionTitle(FarmsPage, "Farms & Illegal Money")
        for key, val in pairs(TheBronx.Farms) do
            if type(val) == "boolean" then
                addToggle(FarmsPage, prettify(key), val, function(state)
                    TheBronx.Farms[key] = state
                end)
            end
        end
    end
    ...
    -- TELEPORT TAB
    addSectionTitle(TeleportPage, "Teleport Locations")
    ...
    TeleportList.Size = UDim2.new(1, -4, 1, -24)
    TeleportList.CanvasSize = UDim2.new(0, 0, 0, 0)
    TeleportList.ScrollBarThickness = 4
    TeleportList.Parent = TeleportPage

    local TLLayout = Instance.new("UIListLayout", TeleportList)
    TLLayout.SortOrder = Enum.SortOrder.LayoutOrder
    TLLayout.Padding = UDim.new(0, 4)

    local TLPadding = Instance.new("UIPadding", TeleportList)
    TLPadding.PaddingTop = UDim.new(0, 2)
    TLPadding.PaddingLeft = UDim.new(0, 2)

    local function updateTLCanvas()
        TeleportList.CanvasSize = UDim2.new(0, 0, 0, TLLayout.AbsoluteContentSize.Y + 8)
    end
    TLLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateTLCanvas)
    updateTLCanvas()

    local names = {}
    if type(TheBronx.TeleportationList) == "table" then
        for name, _ in pairs(TheBronx.TeleportationList) do
    ...
    for _, name in ipairs(names) do
        ...
        Btn.MouseButton1Click:Connect(function()
            ...
            local cf = TheBronx.TeleportationList and TheBronx.TeleportationList[name]
            if hrp and cf then
                hrp.CFrame = cf
                TheBronx.Selected_Location = name
            end
        end)
    end

    -- MISC TAB (PlayerModifications + misc options)
    addSectionTitle(MiscPage, "Player Modifications")
    if type(TheBronx.PlayerModifications) == "table" then
        for key, val in pairs(TheBronx.PlayerModifications) do
            if type(val) == "boolean" then
                addToggle(MiscPage, prettify(key), val, function(state)
                    TheBronx.PlayerModifications[key] = state
                end)
            end
        end
    end
    ...
    Config.TheBronx = TheBronx
    Config.WorldVisuals = WorldVisuals
    Config.MiscSettings = MiscSettings
    Config.Aimlock = Aimlock
    ...
    -- Optional RightShift keybind to toggle UI
