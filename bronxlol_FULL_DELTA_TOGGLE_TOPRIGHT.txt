...
-- =========================
-- UI-ONLY FORCE RED PATCH
-- Affects ONLY ScreenGui elements (UI), not the game world
-- =========================
do
    local RED = Color3.fromRGB(0, 255, 0)

    local function looksWhite(c)
        return typeof(c) == "Color3" and c.R >= 0.85 and c.G >= 0.85 and c.B >= 0.85
    end

    local function apply(v)
        if v:IsA("TextLabel") or v:IsA("TextButton") or v:IsA("TextBox") then
            if looksWhite(v.TextColor3) then v.TextColor3 = RED end
            if v.TextStrokeTransparency < 1 and looksWhite(v.TextStrokeColor3) then
                v.TextStrokeColor3 = RED
            end
        elseif v:IsA("UIStroke") then
            if looksWhite(v.Color) then v.Color = RED end
        elseif v:IsA("ImageLabel") or v:IsA("ImageButton") then
            if looksWhite(v.ImageColor3) then v.ImageColor3 = RED end
        elseif v:IsA("Frame") then
            if looksWhite(v.BackgroundColor3) then v.BackgroundColor3 = RED end
        end
    end
...
local window = library:window({name = "Ski Hub", suffix = "", gameInfo = string.format(" Best Tha Bronx 3 Script ", Game_Name:lower())})
